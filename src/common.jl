...
"""
This function is a generic implementation of oversampling. It takes a function that oversamples a set of points belonging to the same class and applies it to each set of points belonging to the same class in the given observations matrix `X` and labels vector `y`.
...
- `oversample_per_class`: A function that takes a matrix of observations and a number of new observations to generate and returns a matrix where each column is a new observation generated by the oversampling method
...
"""
function generic_oversample(X::AbstractMatrix{<:AbstractFloat}, y, oversample_per_class; ratios=nothing, kwargs...)
...
"""
Takes a function that takes X, y and key-word argument but only works with abstract matrices and generalizes it to work with tables.
To do this, it converts the table to a matrix, applies the function, then converts the matrix back to a table.

# Arguments
- `func::Function`: A function that takes a matrix of observations and a vector of labels and returns a matrix of oversampled observations and a vector of oversampled labels
...
# Returns
- `Dict`: A dictionary mapping each class to the number of extra samples needed for that class to achieve the given ratio relative to the majority class.
"""
