...
const ERR_INVALID_REF = "reference must be either \"majority\" or \"minority\""
...
const INFO_BORDERLINE_PTS(y1_stats) =
    "After filtering, the mapping from each class to number of borderline points is ($y1_stats)."

### SMOTENC
const ERR_BAD_MIXED_COL_TYPES(bad_cols, bad_types) =
    "Error: Columns at indices $(bad_cols) are neither categorical nor continuous.\n Categorical columns must have scitype Multiclass or OrderedFactor and continuous columns must have scitype Count or Continuous.\n However, said columns have scitypes $(bad_types)."
# May need to do similar checks for matrices but they are just numbers
const ERR_WRNG_TREE(knn_tree) =
    "Error: Expected KNN tree to be either \"Brute\" or \"Ball\" but given is \"$knn_tree\""

### SMOTEN and RandomWalk
const ERR_BAD_NOM_COL_TYPES(bad_cols, bad_types) =
    "Error: Columns at indices $(bad_cols) are not categorical.\n Categorical columns must have scitype Multiclass or OrderedFactor.\n However, said columns have scitypes $(bad_types)."

### Cluster Undersampling
const ERR_INVALID_MODE = "Error: mode must be either \"center\" or \"nearest\" "

### ENN Undersampling
const ERR_KEEP_CONDS = "Error: keep_condition must be one of: \"exists\", \"mode\", \"only mode\", \"all\""
