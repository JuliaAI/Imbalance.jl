...
const ERR_NONPOS_K(k) = "Error: Number of nearest neighbors `k` must be greater than 0 but got $k."
const WRN_K_TOO_BIG(k, n) =
    "k=$k is  larger than or equal to the number of samples for the data which is ($n). Will set k=$n-1"

### BorderlineSMOTE1
const ERR_NONPOS_M(m) = "Error: Number of nearest neighbors `m` must be greater than 0 but got $m."
