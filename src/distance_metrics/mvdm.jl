...
"""
function precompute_value_encodings(X::AbstractMatrix{<:Integer}, y::AbstractVector)
    classes = sort(unique(y))
    num_classes = length(classes)
    num_categories_per_col = [length(1:maximum(X[:, col])) for col in 1:size(X, 2)]
    num_cols = size(X, 2)
    ...
    for col in 1:num_cols
        ...
        for category_ind in 1:size(mvdm_encoder[col], 2)
