...
"""
Plot the data before and after oversampling in a 4x4 grid where the first row is the histogram of the labels before and after oversampling and the second row is the scatter plot of the observations before and after oversampling.
...
- `hist_only::Bool`: If true, only plot the histograms of the labels before and after oversampling. If false, plot the histograms and the scatter plots of the observations before and after oversampling.
"""
function plot_data(y_before, y_after, X_before, X_after; hist_only=false)
    ...
    if !hist_only
        # Scatter plot
        p3 = scatter(X_before[:, 1], X_before[:, 2], xlabel="X1", ylabel="X2", title="", legend=false,
                    color=[class_colors[label_map[yi]] for yi in y_before])
        p4 = scatter(X_after[:, 1], X_after[:, 2], xlabel="X1", ylabel="X2", title="", legend=false,
