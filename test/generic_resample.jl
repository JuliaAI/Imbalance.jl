...
using Imbalance:
    generic_undersample, generic_oversample, generic_clean_per_class, filter_modifier
...
@testset "generic_undersample" begin
    ...
    Xover, yover =
        generic_undersample(X, y, undersample_per_class; ratios = Dict([1 => 1.0]))
...
@testset "generic_oversample" begin
    ...
    @test size(Xover, 1) ==
          size(X, 1) + size(X[y.==0, :], 1) * 2 -
          (size(X[y.==1, :], 1) + size(X[y.==2, :], 1))
end
