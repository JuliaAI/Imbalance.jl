...
@testset "cluster undersampler" begin
    ...
    X_under, y_under =
        cluster_undersample(X, y; ratios = Dict(0 => 0.8, 1 => 1.2, 2 => 1.0), rng = Random.Xoshiro(121))
    ...
    center = kmeans(X_c0, n; maxiter = 100, rng=Random.Xoshiro(121)).centers[:, 2]
    tree = BallTree(X_c0)
    ...
    @test !issubset(Set(eachrow(X_under)), Set(eachrow(X)))
    @test X_under[y_under.==0, :]' ≈ kmeans(X_c0, n; maxiter = 100, rng=Random.Xoshiro(121)).centers
