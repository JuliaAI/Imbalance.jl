...
@testset "get_collinear_point" begin
    x₁ = [1.0, 2.0, 3.0]
    x₂ = [4.0, 5.0, 6.0]
    collinear = get_collinear_point(x₁, x₂; rng)
...
@testset "generate_new_smote_point" begin
    X = transpose([1.0 1.0; 2.0 2.0; 3.0 3.0; 4.0 4.0; 5.0 5.0])
    tree = KDTree(X)
    k = 2
    new_point = generate_new_smote_point(X, tree; k, rng)
    @test any(is_in_between(getobs(X, i), new_point, getobs(X, j)) 
    for i in 1:numobs(X) for j in 1:numobs(X))
end
